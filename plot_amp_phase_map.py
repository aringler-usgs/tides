...
for idx, comp in enumerate(['Z', 'N', 'E']):
    ...
    ax = fig.add_subplot(3,2,2*idx+1, projection = ccrs.Robinson())
    ax.coastlines()
    ax.add_feature(cart.feature.LAND, zorder=2, edgecolor='k')
    ax.set_global()
    ax.coastlines()
    ax.set_title(letters[2*idx] + ' ' + comp +  ' component', loc='left')
    im = ax.scatter(alons, alats, c=vals, transform=ccrs.Geodetic(), zorder=3, vmin=-10, vmax = 10)
    
    ax = fig.add_subplot(3,2,2*idx+2, projection = ccrs.Robinson())
    ax.set_title(letters[2*idx + 1], loc='left')
    ax.coastlines()
    ax.add_feature(cart.feature.LAND, zorder=2, edgecolor='k')
    ax.set_global()
    ax.coastlines()
    im = ax.scatter(alons, alats, c=lens, transform=ccrs.Geodetic(), zorder=3, vmin =0., vmax = 50.)
...
plt.savefig('Map_Phase.png', format='PNG', dpi=400)
